...
## 📝 작업 로그
### 2026-07-18
- 생활 실용 20편 네이버 포스트 복사용 HTML 파일 일괄 변환 및 생성
### 2026-07-10
- 이전글/다음글 하단 여백 축소
- 태그 하단 여백 확대

### 2026-07-09
- TokyoKorean 메인 OG 이미지 오류 수정 배포

### 2026-07-04
- 단일 로케일 SSOT 근본 수정 prod
- GSC 404 /ko /ja 리디렉션 수정 prod 배포
...
- **네이버 CTA 3건** 라이브 확인: 개별 `/posts/...` + `utm_source=naver&utm_medium=blog&utm_campaign=tk-broadcast` **전부 정상**. 병목 ≠ 태깅.
- **병목**: 네이버 포스트 노출 부족(예: 7/18 조회 0) · GA4 28d `naver/blog`·`naver/organic` 각 ~1 · direct 92 위주. Gate B-4 **아직 미충족**.
- **기술·콘텐츠 전면 재검증 (라이브)**: ads.txt · robots · 플레이스홀더0 · 내부링크0깨짐 · 본문 7k–13k자 · about/contact/privacy/terms · AdSense/consent/gtag · 모바일 · YMYL4편 면책·1인칭 · `/ko``/ja` 308→200 — **PASS**. 사이트 신규 포스팅 **불필요**.
- **열어둔 이슈**: GA4「404 페이지」~40조회 발생 URL 미특정 (내부 링크·locale 리다이렉트 원인 아님) → GA4에서 이전 경로 스크린샷 필요.
- **8/3 신청 목표** 유지. B-4는 팀 안전장치(공식 트래픽 요건 아님) — 8/3에 레퍼럴 미달이면 Joseph이 신청/대기 재판단. **승인 보장 아님**.
